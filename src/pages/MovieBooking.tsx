      ...
      navigate("/utilities/result", {
        state: {
          result: {
            ...
            title: `Đặt vé xem phim: ${selectedMovie.title}`,
            time: new Date().toLocaleString("vi-VN"),
            fee: "0 ₫",
            transactionId: result.transactionId,
            details: [
              { label: "Rạp phim", value: selectedCinema.name },
              { label: "Phim", value: selectedMovie.title },
              { label: "Ngày chiếu", value: selectedShowtime.date },
              { label: "Giờ chiếu", value: selectedShowtime.time },
              { label: "Phòng", value: `Phòng ${selectedShowtime.room}` },
              { label: "Ghế đã chọn", value: selectedSeats.join(", ") },
              { label: "Số lượng vé", value: `${selectedSeats.length} vé` },
              { label: "Mã đặt vé", value: result.bookingId },
            ],
          },
