...
// Global seeded random instance (seed: 54321)
...
function generateHotels(cityKey, cityName, count, priceRange, isVN = true) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const amenities = amenitiesOptions[Math.floor(random() * amenitiesOptions.length)];
    const prefix = prefixes[i % prefixes.length];
    const suffix = suffixes[Math.floor(random() * suffixes.length)];
    
    // Price based on stars
    const basePrice = priceRange[0] + (stars - 3) * ((priceRange[1] - priceRange[0]) / 3);
    const priceVariation = (random() - 0.5) * 0.3 * basePrice;
    ...
    hotels.push({
      ...
      lon: 0,
      stars,
      rating: Number((3.5 + random() * 1.4).toFixed(1)),
      priceFrom: isVN ? price : Math.round(price / 23000), // USD for international
      distanceToCenterKm: Number((0.3 + random() * 5).toFixed(1)),
      amenities,
      images: [],
    });
  }
...
const VN_HOTELS = [
  ...
  ...generateHotels("VN_HN", "Hanoi", 50, [500000, 2500000], true).map((h, i) => ({
    ...h,
    lat: 21.0285 + (random() - 0.5) * 0.05,
    lon: 105.8542 + (random() - 0.5) * 0.05,
  })),
  
  // TP.HCM - 50 hotels
  ...generateHotels("VN_HCM", "Saigon", 50, [550000, 2800000], true).map((h, i) => ({
    ...h,
    lat: 10.7758 + (random() - 0.5) * 0.05,
    lon: 106.7009 + (random() - 0.5) * 0.05,
  })),
  
  // Đà Nẵng - 40 hotels
  ...generateHotels("VN_DN", "Da Nang", 40, [450000, 2200000], true).map((h, i) => ({
    ...h,
    lat: 16.0704 + (random() - 0.5) * 0.04,
    lon: 108.2245 + (random() - 0.5) * 0.04,
  })),
  
  // Nha Trang - 35 hotels
  ...generateHotels("VN_NT", "Nha Trang", 35, [400000, 2000000], true).map((h, i) => ({
    ...h,
    lat: 12.2388 + (random() - 0.5) * 0.03,
    lon: 109.1967 + (random() - 0.5) * 0.03,
  })),
  
  // Phú Quốc - 30 hotels
  ...generateHotels("VN_PQ", "Phu Quoc", 30, [600000, 3000000], true).map((h, i) => ({
    ...h,
    lat: 10.2899 + (random() - 0.5) * 0.03,
    lon: 103.9840 + (random() - 0.5) * 0.03,
  })),
  
  // Hội An - 30 hotels
  ...generateHotels("VN_HA", "Hoi An", 30, [350000, 1800000], true).map((h, i) => ({
    ...h,
    lat: 15.8801 + (random() - 0.5) * 0.02,
    lon: 108.3380 + (random() - 0.5) * 0.02,
  })),
  
  // Huế - 25 hotels
  ...generateHotels("VN_HUE", "Hue", 25, [300000, 1500000], true).map((h, i) => ({
    ...h,
    lat: 16.4637 + (random() - 0.5) * 0.02,
    lon: 107.5909 + (random() - 0.5) * 0.02,
  })),
  
  // Đà Lạt - 30 hotels
  ...generateHotels("VN_DL", "Da Lat", 30, [350000, 1600000], true).map((h, i) => ({
    ...h,
    lat: 11.9404 + (random() - 0.5) * 0.02,
    lon: 108.4583 + (random() - 0.5) * 0.02,
  })),
  
  // Vũng Tàu - 25 hotels
  ...generateHotels("VN_VT", "Vung Tau", 25, [400000, 1800000], true).map((h, i) => ({
    ...h,
    lat: 10.3460 + (random() - 0.5) * 0.02,
    lon: 107.0843 + (random() - 0.5) * 0.02,
  })),
  
  // Hạ Long - 25 hotels
  ...generateHotels("VN_HL", "Ha Long", 25, [450000, 2000000], true).map((h, i) => ({
    ...h,
    lat: 20.9517 + (random() - 0.5) * 0.02,
    lon: 107.0748 + (random() - 0.5) * 0.02,
  })),
];
...
function generateRooms(hotelId, stars, basePrice, isVN = true) {
  ...
  availableTypes.forEach((roomType, idx) => {
    ...
    const refundable = idx < 2 ? random() > 0.3 : random() > 0.6;
